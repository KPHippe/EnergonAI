    ...
    def __init__(
        ...
        if last:
            self.norm = LayerNorm1D(
                normalized_shape=hidden_size, eps=layernorm_epsilon, dtype=dtype
            )
            self.head = LMHead1D(
                hidden_size=hidden_size,
                vocab_size=vocab_size,
    ...
    def forward(
        ...
        for _ in range(cur_len, tgt_len):
            ...
            if attention_mask is not None:
                ...
                attention_unfold_mask = attention_unfold_mask.to(
            ...
            if torch.all(hidden_states == 50256):
                ...
                attention_mask = torch.cat(
                    (
                        attention_mask,
                        torch.ones(batch_size, 1, device=torch.cuda.current_device()),
                    ),
                    1,
                )
...
def partition_uniform(num_items, pipeline_parallel_size):
    ...
    assert (
        num_items % pipeline_parallel_size == 0
...
def create_pipeline_model(depth: int = 48, layer_partitions=None, **model_kwargs):
    logger = get_dist_logger("energonai")
    pipeline_size = 0
    pipeline_rank = 0
    ...
    parts = (
        partition_uniform(depth, pipeline_size)[pipeline_rank]
        if layer_partitions is None
        else layer_partitions
    )
...
def gpt2_small(**kwargs):
    model_kwargs = dict(
        hidden_size=768, depth=12, num_heads=12, is_decoder=True, **kwargs
    )
    return create_pipeline_model(**model_kwargs)
...
def gpt2_large(**kwargs):
    model_kwargs = dict(
        hidden_size=1536, depth=36, num_heads=12, is_decoder=True, **kwargs
    )
    return create_pipeline_model(**model_kwargs)
...
def gpt2_8B(**kwargs):
    model_kwargs = dict(
        hidden_size=3072, depth=72, num_heads=24, is_decoder=True, **kwargs
    )
    return create_pipeline_model(**model_kwargs)
...
def gpt3(**kwargs):
    model_kwargs = dict(
        hidden_size=12288, depth=12, num_heads=96, is_decoder=True, **kwargs
    )
    return create_pipeline_model(**model_kwargs)
...
def bert_small(**kwargs):
    model_kwargs = dict(
        hidden_size=768, depth=12, num_heads=12, is_decoder=False, **kwargs
    )
    return create_pipeline_model(**model_kwargs)
...
def bert_large(**kwargs):
    model_kwargs = dict(
        hidden_size=1024, depth=24, num_heads=16, is_decoder=False, **kwargs
    )
    return create_pipeline_model(**model_kwargs)
...
def bert_8B(**kwargs):
    model_kwargs = dict(
        hidden_size=3072, depth=72, num_heads=24, is_decoder=False, **kwargs
    )
    return create_pipeline_model(**model_kwargs)
...
def bert_175B(**kwargs):
    model_kwargs = dict(
        hidden_size=12288, depth=96, num_heads=96, is_decoder=False, **kwargs
    )
    return create_pipeline_model(**model_kwargs)
