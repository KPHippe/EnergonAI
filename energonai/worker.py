...
class Worker:
    ...
    # TODO: I think this is where the forward function gets run.
    # See if we can add more checks to see where we can use the generate function
    def _forward(self, inputs: Any) -> Any:
        ...
        if isinstance(inputs, (tuple, list)):
            ...
            self.logger.info("Input is mappable")
            if hasattr(self.model, "generate"):
                self.logger.info("Huggingface generate function")
                outputs = self.model.generate(**inputs)

            else:
                outputs = self.model(**inputs)

        else:  # TBH not sure what to do here, it should be a list or dict
            outputs = self.model(inputs)
